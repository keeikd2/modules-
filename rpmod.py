...
class RPMod(loader.Module):
	...
	async def client_ready(self, client, db):
		...
		if not self.db.get('RPMod', 'rpcomands', False):
			comands = {'лизь': 'лизнула', 'кусь': 'кусьнула', 'поцеловать': 'поцеловала', 'выебать': 'выебала', 'трахнуть': 'трахнула', 'выпороть': 'выпорола', 'шлепнуть': 'шлепнула', 'шлёпнуть': 'шлёпнула', 'отлизать': 'отлизала', 'прижать': 'прижала', 'погладить': 'погладила', 'обнять': 'обняла'}
	...
	async def rpnickcmd(self, message):
		...
		nick = ''
		for x in args:
			if x in emoji.EMOJI_DATA['en'].keys(): lst.append(x)
			if x not in emoji.EMOJI_DATA['en'].keys(): nick+=x
